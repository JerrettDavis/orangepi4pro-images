...
grep -q '^LABEL ubuntu-nvme$' /boot/extlinux/extlinux.conf || fail 'extlinux selector label missing'
...
awk '
  /^LABEL ubuntu-nvme$/ { in_selector = 1; next }
  /^LABEL / { in_selector = 0 }
  in_selector && /^  INITRD ..\/uInitrd-orangepi4pro-bootselect$/ { found = 1 }
  END { exit found ? 0 : 1 }
' /boot/extlinux/extlinux.conf || fail 'extlinux ubuntu-nvme must use selector initrd'
